...
# Cubic-BÃ©zier control distance for approximating a quarter circle / ellipse.
# Distance from corner to control point along the tangent, expressed as a
# fraction of the radius. Standard "magic number" for a 90Â° arc (max error
# ~0.027% of the radius).
...
def _build_round_rect_custgeom(w: float, h: float, rx: float, ry: float) -> str:
    ...
    Used when ``<rect>`` has rx â‰  ry, which DrawingML's preset ``roundRect``
    cannot express (the preset takes a single ``adj`` shared by all four
    corners and is implicitly symmetric). Each 90Â° elliptical arc is
    approximated by one cubic BÃ©zier â€” within 0.03% of the true ellipse, far
    ...
    reality â€” rx â‰  ry has no single "radius" to drag â€” and remains far
    ...
    parts = [
        ...
        # Top-right corner: (W-Rx, 0) â†’ (W, Ry)
        cubic(
        ...
        # Bottom-right corner: (W, H-Ry) â†’ (W-Rx, H)
        cubic(
        ...
        # Bottom-left corner: (Rx, H) â†’ (0, H-Ry)
        cubic(
...
def convert_rect(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    for now â€” current corpora contain none, but the branch keeps callers from
    ...
    if rx > 0 and abs(rx - ry) < 0.5:
        # Symmetric corners â†’ native PowerPoint rounded rectangle. adj is
        ...
        # Asymmetric corners (rx != ry) â†’ DrawingML has no preset for
        # elliptical-corner rectangles, so emit a custGeom with one cubic
        # BÃ©zier per 90Â° arc. We lose the prstGeom roundRect adjustment
...
def _is_donut_circle(elem: ET.Element, ctx: ConvertContext) -> bool:
    ...
    # Real donut arcs need sw/r >= 0.15 (e.g. sw=40 on r=100 â†’ 0.40).
    if sw / r < 0.15:
        return False
...
def convert_line(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if has_marker:
        ...
        #   headEnd  â†’ placed at the start of the line = (x1, y1)
        #   tailEnd  â†’ placed at the end   of the line = (x2, y2)
        # We set flipH / flipV so that the preset start/end align with the
        # original SVG endpoints:
        #   default  (no flip)  : top-left  â†’ bottom-right  (x1â‰¤x2, y1â‰¤y2)
        #   flipH               : top-right â†’ bottom-left   (x1>x2, y1â‰¤y2)
        #   flipV               : bottom-left â†’ top-right   (x1â‰¤x2, y1>y2)
        #   flipH + flipV       : bottom-right â†’ top-left   (x1>x2, y1>y2)
        # ----------------------------------------------------------------
        w = abs(x2 - x1)
        h = abs(y2 - y1)
        # DrawingML requires ext cx/cy â‰¥ 1 EMU
...
def _collect_tspan_runs(
    ...
    Order: tspan.text â†’ (each nested child tspan's runs â†’ that child's tail under THIS tspan's attrs).
    """
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    # box so its center lands where SVG would place the rotated visual center â€”
    ...
    # but NO spAutoFit â€” otherwise PowerPoint expands the frame to fit a
...
def _resolve_clip_geometry(
    ...
    Supports:
      - circle / ellipse  â†’ prstGeom ellipse
      - rect with rx/ry   â†’ prstGeom roundRect
      - path / polygon     â†’ custGeom
...
def _read_image_size(data: bytes) -> tuple[int | None, int | None]:
    ...
    Returns ``(None, None)`` on any failure â€” callers fall back to the
...
def _compute_slice_src_rect(
    ...
    units of 1/1000 of a percent (0â€“100000).
    ...
    visible_w = box_w / scale  # â‰¤ img_w
    visible_h = box_h / scale  # â‰¤ img_h
...
def _resolve_image_src_rect(
    ...
    if align == 'none' or mode != 'slice':
        return ''  # meet handled by frame fit; none â†’ stretch is correct per SVG spec
...
def _resolve_image_meet_fit(
    ...
    PowerPoint has no native ``meet`` semantic â€” ``<a:stretch><a:fillRect/>``
    ...
    Returns ``None`` when the adjustment is not applicable:
      ...
      - align is ``none`` (SVG spec says: stretch â€” do not adjust)
    ...
    if abs(fit_w - box_w) < 0.5 and abs(fit_h - box_h) < 0.5:
        return None  # already matches â€” no adjustment
...
def convert_image(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    # Resolve clip-path â†’ DrawingML geometry
    clip_geom = _resolve_clip_geometry(elem, ctx, raw_x, raw_y, raw_w, raw_h)

    # Resolve preserveAspectRatio="<align> slice" â†’ DrawingML <a:srcRect>.
...
# Inverse of pptx_to_svg/pic_to_svg.py:101-113 â€” that path writes a cropped
