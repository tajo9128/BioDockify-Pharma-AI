...
This guide provides comprehensive technical details for configuring and utilizing external tool providers through the Model Context Protocol (MCP) with BioDockify AI. This allows BioDockify AI to leverage tools hosted by separate local or remote MCP-compliant servers.

> [!NOTE]
> For a quick start guide on adding MCP servers through the UI, see [MCP Setup](../guides/mcp-setup.md).

> [!NOTE]
> This guide covers BioDockify AI as an MCP **client**. To expose BioDockify AI as an MCP **server**, see [Connectivity → MCP Server](connectivity.md#mcp-server-connectivity).

## MCP Server Types

BioDockify AI supports three main types of MCP servers:

1.  **Local Stdio Servers**: Local executables that BioDockify AI communicates with via standard input/output (stdio).
...
## How BioDockify AI Consumes MCP Tools

BioDockify AI discovers and integrates MCP tools dynamically through the following process:

1.  **Configuration**: MCP servers are defined in the BioDockify AI configuration, primarily through the Settings UI.
2.  **Saving Settings**: When saved via the UI, BioDockify AI updates `usr/settings.json`, specifically the `"mcp_servers"` key.
3.  **Server Startup**: BioDockify AI initializes configured MCP servers (stdio) or connects to them (remote). For `npx`/`uvx` based servers, the first run downloads packages.
4.  **Tool Discovery**: Upon initialization, BioDockify AI connects to each enabled MCP server and queries for available tools, descriptions, and parameters.
...
6.  **Tool Invocation**: When the LLM requests an MCP tool, BioDockify AI's `process_tools` method (`mcp_handler.py`) routes the request to the appropriate MCP server.
...
### BioDockify AI in Docker, MCP Server on Host
...
- Final tool name in BioDockify AI: `sequential_thinking.run_chain`
...
- BioDockify AI maintains persistent connections to remote servers
...
BioDockify AI can chain these tools automatically based on task requirements.
...
- Test with BioDockify AI before production
...
- [Connectivity: MCP Server](connectivity.md#mcp-server-connectivity) - Exposing BioDockify AI as MCP server
