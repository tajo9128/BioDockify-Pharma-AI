# BioDockify AI — Frontend Modals (stacked `openModal/closeModal`)

This document covers the stacked modal system used by BioDockify AI’s frontend, implemented in:
...
BioDockify AI uses a custom `<x-component path="...">` tag that is populated by the component loader.
...
BioDockify AI can open modals from many places (sidebar, settings, message actions), and sometimes a modal opens another modal.
