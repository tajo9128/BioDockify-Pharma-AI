---
name: linux-desktop
description: Use when the user asks BioDockify AI to operate the built-in Linux Desktop, XFCE apps, LibreOffice GUI apps, file manager, terminal, or visual desktop workflows.
version: "0.3.0"
...
Use the Desktop as a full Linux GUI when the user explicitly needs a visual workflow, an installed desktop app, or manual layout polish that is awkward through structured file edits alone. BioDockify AI may warm the persistent Desktop runtime during initial startup, but visible Desktop/canvas use remains opt-in. The Desktop is opt-in at the UI level: do not open the canvas just because the user asks for a document. Use structured tools first for deterministic content changes, then use the Desktop for inspection, GUI-only actions, and final visual confirmation.
...
6. Use the external BioDockify AI Browser for web browsing. Do not launch an operating-system browser in this version.
...
Use the helper script when the Desktop is already open and you need reliable app launches, clicks, keystrokes, or window checks from the agent shell. In the live BioDockify AI runtime, prefer the absolute path so the command works from any current directory:
...
- `Workdir` -> configured BioDockify AI workdir (default `/a0/usr/workdir`)
...
- `Workdir`: graphical file management with Thunar at the configured BioDockify AI workdir (default `/a0/usr/workdir`).
- `Terminal`: shell work inside the BioDockify AI runtime.
...
- Do not treat closing a document tab as closing the whole Desktop. The Desktop is persistent while BioDockify AI is running.
