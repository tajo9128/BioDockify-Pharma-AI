...
try:
    from orchestration.planner.orchestrator import (
...
try:
    ...
    logging.getLogger("biodockify_api").warning(
        f"Agent Enhanced routes not loaded: {e}"
    )
...
try:
    ...
    logging.getLogger("biodockify_api").warning(
        f"Enhanced Project routes not loaded: {e}"
    )
        ...
        if not (is_valid_origin or is_valid_referer):
            ...
            if origin or referer:
                return JSONResponse(
                    status_code=403,
                    content={
                        "detail": "CSRF verification failed: Invalid origin or referer"
                    },
                )
...
async def verify_emergency_access(request: Dict[str, str]):
    ...
    if success:
        ...
        raise HTTPException(
            status_code=403, detail="Invalid or Expired Emergency Token"
        )
...
async def agent_chat(request: AgentRequest):
    ...
    try:
        ...
        logger.info(
            f"BioDockify AI (v1.9): Processing chat request (len={len(request.message)})"
        )
        agent = get_biodockify_ai()

        reply = await agent.process_chat(request.message)

        return {"reply": reply, "provider": "biodockify-agent0-v1.9", "enhanced": True}
    except Exception as e:
...
async def get_ollama_models():
    ...
    base_url = ollama_url.rstrip('/').replace('/v1', '').replace('/v1/', '/')
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {"models": [], "status": "error", "message": f"Failed to connect: {resp.status_code}"}
    except Exception as e:
...
async def set_agent_goal(request: AgentGoal, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to verify AI provider: {str(e)}"
        )
    ...
    def run_agent_task(goal: str, mode: str):
        try:
            ...
            if plan is None:
                raise ValueError(
                    "LLM failed to generate a plan. Check AI provider configuration."
                )
...
async def generate_podcast(request: PodcastRequest):
    ...
    try:
        ...
        custom_base_url = cfg.get("ai_provider", {}).get(
        ...
        if custom_key:
            ...
            return {
                "status": "error",
                "error": "No TTS API key configured. Please add an OpenAI-compatible API key in Settings.",
            }
...
async def search_literature_endpoint(req: LitSearchRequest):
    ...
    """
    try:
        config = LiteratureConfig(sources=req.sources, max_results=req.limit)
...
async def synthesize_literature_endpoint(req: LitSynthesizeRequest):
    ...
    try:
        ...
        async def agent_callback(prompt: str) -> str:
            ...
            if not adapter:
                adapter = LLMFactory.get_adapter("ollama", config_obj)
    ...
    try:
        ...
        try:
            ...
            if chunks:
                ...
                library_store.update_metadata(
                    record["id"], {"processed": True, "chunk_count": chunk_count}
                )
        except Exception as e:
            logger.warning(f"Vector indexing failed for {file.filename}: {e}")
            vector_status = "failed"
            library_store.update_metadata(
                record["id"], {"processed": False, "index_error": str(e)}
            )
...
async def delete_file(file_id: str):
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail="File not found or failed to delete"
        )
...
async def startup_event():
    ...
    def detect_ai_providers():
        ...
        # Check Ollama - first local, then Docker host
        ollama_urls = ["127.0.0.1", "host.docker.internal"]
        ollama_port = 11434
        ...
        for host in ollama_urls:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(2)
                    result = s.connect_ex((host, ollama_port))
                    if result == 0:
                        logger.info(f"✅ Ollama detected on {host}:{ollama_port}")
                        ollama_detected = True
                        # Try to list models
                        try:
                            import requests
                            resp = requests.get(f"http://{host}:{ollama_port}/api/tags", timeout=5)
                            if resp.status_code == 200:
                                data = resp.json()
        ...
        if not ollama_detected:
            logger.info(f"❌ Ollama not running on ports {ollama_urls}:{ollama_port}")
        ...
        try:
            ...
            provider_mode = ai_config.get("mode", "")
            
            if provider_mode == "ollama":
                ollama_url = ai_config.get("ollama_url", "http://localhost:11434")
                logger.info(f"📌 Configured provider: Ollama at {ollama_url}")
    ...
    def background_init():
        ...
        try:
            ...
            if hasattr(vs, "model") and vs.model:
                ...
                logger.warning(
                    "Background Init: Model not loaded (dependencies missing?)."
                )
...
async def resource_monitor_middleware(request: Request, call_next):
    ...
    if mem.percent > 95:  # Critical Threshold
        return JSONResponse(
            status_code=503,
            content={
                "detail": "System is under heavy load (Memory > 95%). Please retry later."
            },
        )
...
def run_research_task(task_id: str, title: str, mode: str):
    ...
    try:
        ...
        config = OrchestratorConfig(use_cloud_api=use_cloud, user_persona=user_persona)
        ...
        for attempt in range(MAX_JOB_RETRIES):
            try:
                ...
                logger.warning(
                    f"Task {task_id} failed attempt {attempt + 1}: {job_err}"
                )
...
def health_check_detailed():
    ...
    try:
        ...
        if mode in ["auto", "ollama"]:
            ...
            resp = safe_request(
                "GET", f"{ollama_url}/api/tags", max_retries=1, timeout=1.5
            )
            if resp.status_code == 200:
                status["components"]["ai_core"] = {"status": "ok", "provider": "ollama"}
            else:
                status["components"]["ai_core"] = {
                    "status": "degraded",
                    "message": f"Ollama Unreachable ({resp.status_code})",
                }
        elif mode == "lm_studio":
            ...
            if resp.status_code == 200:
                ...
                status["components"]["ai_core"] = {
                    "status": "degraded",
                    "message": f"LM Studio Unreachable",
                }
        else:
            status["components"]["ai_core"] = {
                "status": "ok",
                "provider": mode,
                "message": "Cloud-based provider active",
            }
...
async def start_research(request: ResearchRequest, background_tasks: BackgroundTasks):
    ...
    if not research_title:
        raise HTTPException(
            status_code=400, detail="Either 'title' or 'topic' is required"
        )
...
async def generate_podcast(request: PodcastRequest):
    ...
    config = load_config()
    surfsense_url = config.get("ai_provider", {}).get(
...
async def agent_execute(request: AgentExecuteRequest):
    ...
    try:
        ...
        # SURFSENSE_VIDEO: Generate Video from Topic/Text
            ...
            final_video = await create_video_summary(
                slide_images, audio_path, output_path=video_path
            )
...
async def chat_with_docs(request: ChatRequest):
    ...
    if not context_list:
        return {
            "answer": "I don't have enough context from your uploaded documents to answer that.",
            "sources": [],
        }
    ...
    try:
        ...
        ollama_url = ai_conf.get("ollama_url", "http://localhost:11434")
        ollama_model = ai_conf.get(
            "ollama_model", "llama3"
        )  # Default to llama3 or mistral if not set
    ...
    return {"answer": answer, "context": context_text, "sources": []}
...
async def generate_chapter_endpoint(req: ThesisRequest):
    ...
    return await thesis_engine.generate_chapter(
        req.chapter_id, req.topic, branch=req.branch, degree=req.degree
    )
    ...
    try:
        ...
        if request.source == "knowledge_base":
            ...
            search_results = rag.search(
                request.search_query, top_k=request.num_slides * 2
            )
...
async def test_connection_endpoint(request: TestConnectionRequest):
    ...
    """
    logger.info(
        f"[DEBUG] test_connection_endpoint called: service_type={request.service_type}, provider={request.provider}, has_key={bool(request.key)}, base_url={request.base_url}, model={request.model}"
    )
    ...
    try:
        ...
        if request.service_type == "llm":
            ...
            if request.provider == "google":
                ...
                adapter = OpenRouterAdapter(
                    request.key, model=request.model or "mistralai/mistral-7b-instruct"
                )
                ...
                adapter = CustomAdapter(
                    ...
                    request.model or "deepseek-chat",
                )
            elif request.provider == "groq":
                adapter = CustomAdapter(
                    request.key,
                    "https://api.groq.com/openai/v1",
                    request.model or "llama-3.3-70b-versatile",
                )
            ...
            # Generic Custom case
                ...
                if not request.base_url:
                    return {
                        "status": "error",
                        "message": "Base URL required for custom provider",
                    }
            ...
            if response:
                ...
                return {
                    "status": "success",
                    "message": f"Connected! Response: {response[:50]}...",
                }
            else:
        ...
        # 2. Elsevier Testing
            ...
            headers = {"X-ELS-APIKey": request.key, "Accept": "application/json"}
            ...
            if resp.status_code == 200:
                return {
                    "status": "success",
                    "message": "Elsevier API Connected via Scopus",
                }
        ...
        # 3. Bohrium Testing (MCP)
            ...
            if result.status_code == 200:
                ...
                return {
                    "status": "error",
                    "message": f"Bohrium Error: {result.status_code}",
                }
        ...
        # 4. Brave Search Testing
            ...
            if result.status_code == 200:
                ...
                return {
                    "status": "error",
                    "message": f"Brave Error: {result.status_code} {result.text[:100]}",
                }

        return {
            "status": "error",
            "message": f"Unknown service type: {request.service_type}",
        }
...
async def verify_user_license(request: AuthRequest):
    ...
    return {
        "success": success,
        "message": message,
        "user": {"name": request.name, "email": request.email} if success else None,
    }
