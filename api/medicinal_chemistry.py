...
class MedicinalChemistryHandler(ApiHandler):
    async def process(self, input: dict, request: Request) -> dict:
        ...
        elif action == "r_group": return self._r_group(input)
        elif action == "analogue_finder": return self._analogue_finder(input)
        return {
            "actions": ["murcko_scaffold", "mmpa", "butina_cluster", "smarts_search",
                        "sa_score", "retrosynthesis", "named_reactions",
                        "protecting_groups", "toxicophore_scan", "stereo_analysis", "r_group", "analogue_finder"],
    ...
    # -- R-group decomposition + analogue enumeration (ported concept from biodockify-web MedChem) --

    COMMON_R_GROUPS = [
        ("H", None), ("Me", "C"), ("Et", "CC"), ("nPr", "CCC"), ("iPr", "C(C)C"),
        ("tBu", "C(C)(C)C"), ("F", "F"), ("Cl", "Cl"), ("Br", "Br"), ("I", "I"),
        ("CF3", "C(F)(F)F"), ("OMe", "CO"), ("OEt", "CCO"), ("OH", "O"),
        ("NH2", "N"), ("NMe2", "N(C)C"), ("CN", "C#N"), ("NO2", "N(=O)=O"),
        ("COMe", "C(C)=O"), ("COOH", "C(=O)O"), ("CONH2", "C(=O)N"),
        ("SO2Me", "S(=O)(=O)C"), ("OCF3", "OC(F)(F)F"), ("cPr", "C1CC1"), ("cHex", "C1CCCCC1"),
        ("Ph", "c1ccccc1"), ("3-pyridyl", "c1cccnc1"), ("2-thienyl", "c1ccs1"),
        ("2-furyl", "c1cco1"), ("SMe", "SC"), ("NHAc", "NC(C)=O"), ("NHMe", "NC"),
        ("morpholine", "N1CCOCC1"), ("piperazine", "N1CCNCC1"),
    ]

    def _auto_labeled_core(self, Chem, lead_mol, core_mol):
        """Build an RGD-compatible core: lead with all non-scaffold branches replaced
        by labeled dummies [*:1], [*:2], ... Returns None if the scaffold does not match."""
        match = lead_mol.GetSubstructMatch(core_mol)
        if not match:
            return None
        match_set = set(match)
        rw = Chem.RWMol(lead_mol)
        label = 1
        # first pass: convert first atom of each off-core branch into a labeled dummy
        for idx in match:
            atom = lead_mol.GetAtomWithIdx(idx)
            for nbr in atom.GetNeighbors():
                nidx = nbr.GetIdx()
                if nidx not in match_set:
                    a = rw.GetAtomWithIdx(nidx)
                    a.SetAtomicNum(0)
                    a.SetIsAromatic(False)
                    a.SetFormalCharge(0)
                    a.SetAtomMapNum(label)
                    # strip the rest of the branch: mark children for deletion below
                    label += 1
        # second pass: delete atoms that are neither scaffold nor dummy
        keep = set()
        for idx in match:
            keep.add(idx)
        for a in rw.GetAtoms():
            if a.GetAtomicNum() == 0 and a.GetAtomMapNum() > 0:
                keep.add(a.GetIdx())
        # a branch dummy keeps only its scaffold neighbor: drop its other neighbors' subtree by deleting non-keep atoms
        for idx in sorted((a.GetIdx() for a in rw.GetAtoms()), reverse=True):
            if idx not in keep:
                rw.RemoveAtom(idx)
        # dummy may still carry its branch children as neighbors -> re-check and delete them
        changed = True
        while changed:
            changed = False
            for a in list(rw.GetAtoms()):
                if a.GetAtomicNum() == 0 and a.GetAtomMapNum() > 0:
                    for nbr in list(a.GetNeighbors()):
                        if nbr.GetAtomicNum() != 0 or nbr.GetAtomMapNum() == 0:
                            if len(a.GetNeighbors()) > 1:
                                rw.RemoveAtom(nbr.GetIdx())
                                changed = True
                            elif nbr.GetAtomicNum() == 0 and nbr.GetAtomMapNum() == 0:
                                rw.RemoveAtom(nbr.GetIdx())
                                changed = True
        try:
            m = rw.GetMol()
    ...
    def _rgd_decompose(self, Chem, core, mols):
        """Run rdRGroupDecomposition; returns (columns dict, matched smiles list)."""
        from rdkit.Chem import rdRGroupDecomposition as rgd
        params = rgd.RGroupDecompositionParameters()
        params.allowNonTemplateAtoms = True
        params.removeHydrogensPostMatch = True
        decomp = rgd.RGroupDecomposition(core, params)
        matched = []
        for smi, m in mols:
            try:
                # Add() returns the item INDEX (0-based) on success — 0 is valid!
                if decomp.Add(m) >= 0:
                    matched.append(smi)
            except Exception:
                continue
        if not matched:
            raise ValueError("No molecule matched the scaffold")
        decomp.Process()
        return decomp.GetRGroupsAsColumns(), matched

    def _r_group(self, input):
        """R-group decomposition: molecules + scaffold -> R-table (substituent frequency per position)."""
        ok, mods = _rdkit()
        if not ok:
            return {"error": "RDKit not available"}
        Chem = mods[0]
        ...
        if not molecules:
            return {"error": "molecules required (array of SMILES)"}
        if not scaffold:
            return {"error": "scaffold required (SMILES)"}
        ...
        try:
            columns, matched = self._rgd_decompose(Chem, core, list(zip(kept, mols)))
        except Exception as e:
            return {"error": f"R-group decomposition failed: {e}"}

        r_keys = [k for k in columns.keys() if k != "Core"]
        n = len(columns.get("Core", []))
        r_table, per_molecule = {}, []
        for i in range(n):
            row = {"smiles": matched[i] if i < len(matched) else f"mol_{i}"}
            for k in r_keys:
                rsmi = Chem.MolToSmiles(Chem.RemoveHs(columns[k][i]))
                row[k] = rsmi
                r_table.setdefault(k, {})
                r_table[k][rsmi] = r_table[k].get(rsmi, 0) + 1
            per_molecule.append(row)
        for k in r_table:
            r_table[k] = dict(sorted(r_table[k].items(), key=lambda kv: -kv[1]))

        return {
            "n_matched": len(matched), "n_input": len(mols),
            "scaffold": Chem.MolToSmiles(core),
            "r_positions": r_keys, "r_table": r_table,
            "per_molecule": per_molecule[:200],
            "note": "R-table shows substituent frequency at each labeled position - the core of SAR discussions.",
        }

    def _attach_fragment(self, Chem, core_mol, dummy_idx, frag_smi):
        """Attach fragment (first atom = attachment) at the dummy atom position.

        frag_smi None => cap with H (remove dummy). Returns sanitized product or None.
        """
        core_rw = Chem.RWMol(core_mol)
        if frag_smi is None:  # H
            atom = core_rw.GetAtomWithIdx(dummy_idx)
            atom.SetAtomicNum(1)
        ...
        if frag is None:
            return None
        neighbors = [n.GetIdx() for n in core_rw.GetAtomWithIdx(dummy_idx).GetNeighbors()]
        if not neighbors:
            return None
        attach_atom_idx = core_rw.GetNumAtoms()
        core_rw.InsertMol(frag)
        core_rw.RemoveAtom(dummy_idx)
        # neighbor indices shift by -1 if they were after the dummy
        nbr = neighbors[0] - (1 if neighbors[0] > dummy_idx else 0)
        try:
            core_rw.AddBond(nbr, attach_atom_idx, Chem.BondType.SINGLE)
            m = core_rw.GetMol()
            Chem.SanitizeMol(m)
            return m
        except Exception:
    ...
    def _analogue_finder(self, input):
        """Enumerate analogues of a lead at one R position using common substituents.

        Auto-derives the Murcko scaffold (labeled with dummies) if not given.
        SMILES-level substitution on the decomposed core keeps chemistry exact.
        """
        ok, mods = _rdkit()
        if not ok:
            return {"error": "RDKit not available"}
        Chem, AllChem, Descriptors, DS, rdMD, MurckoScaffold, Butina = mods
        try:
            from rdkit.Chem import Crippen
        except ImportError:
            Crippen = None

        lead = input.get("smiles", input.get("lead", ""))
        lead_mol = Chem.MolFromSmiles(lead) if lead else None
        if lead_mol is None:
            return {"error": "Valid lead smiles required"}

        scaffold = input.get("scaffold", "") or MurckoScaffold.MurckoScaffoldSmiles(mol=lead_mol)
        if not scaffold:
            return {"error": "Could not derive Murcko scaffold; provide scaffold explicitly"}

        core_plain = Chem.MolFromSmiles(scaffold)
        if core_plain is None:
            return {"error": f"Invalid scaffold SMILES: {scaffold}"}
        labeled = self._auto_labeled_core(Chem, lead_mol, core_plain)
        if labeled is None:
            return {"error": "Scaffold does not match the lead molecule"}
        try:
            columns, matched = self._rgd_decompose(Chem, labeled, [(lead, lead_mol)])
        except Exception as e:
            return {"error": f"Decomposition failed: {e}"}

        r_keys = [k for k in columns.keys() if k != "Core"]
        if not r_keys:
            return {"error": "Scaffold has no R positions"}

        target_pos = input.get("position", r_keys[0])
        if target_pos not in r_keys:
            return {"error": f"position must be one of {r_keys}"}

        import re as _re

        def _num(key):
            return _re.search(r"\d+", key).group(0)

        def _dummy_by_mapnum(mol, n):
            for a in mol.GetAtoms():
                if a.GetAtomicNum() == 0 and a.GetAtomMapNum() == int(n):
                    return a.GetIdx()
            return None

        def _join(Chem, base_mol, base_dummy_idx, frag_mol):
            """Join fragment at the dummy position; fragment's own dummy
            provides its attachment atom. Exact bond-level chemistry."""
            combined = Chem.CombineMols(base_mol, frag_mol)
            rw = Chem.RWMol(combined)
            n_base = base_mol.GetNumAtoms()
            b_nbrs = [x.GetIdx() for x in rw.GetAtomWithIdx(base_dummy_idx).GetNeighbors()]
            # locate the fragment's dummy in combined coordinates
            f_dummy = None
            for a in rw.GetAtoms():
                if a.GetIdx() >= n_base and a.GetAtomicNum() == 0:
                    f_dummy = a.GetIdx()
                    break
            if f_dummy is None or not b_nbrs:
                return None
            f_nbrs = [x.GetIdx() for x in rw.GetAtomWithIdx(f_dummy).GetNeighbors()]
            if not f_nbrs:
                return None
            try:
                rw.AddBond(b_nbrs[0], f_nbrs[0], Chem.BondType.SINGLE)
                for di in sorted([base_dummy_idx, f_dummy], reverse=True):
                    rw.RemoveAtom(di)
                m = rw.GetMol()
                Chem.SanitizeMol(m)
                return m
        ...
        # lead's own substituent at each position (fragment mol carries the dummy)
        lead_r = {k: Chem.MolToSmiles(columns[k][0]) for k in r_keys}

        # base: core with the lead's fragments rejoined everywhere EXCEPT target
        base_mol = columns["Core"][0]
        for k in r_keys:
            if k == target_pos:
                continue
            di = _dummy_by_mapnum(base_mol, _num(k))
            if di is None:
                return {"error": f"dummy for {k} not found in core"}
            base_mol = _join(Chem, base_mol, di, columns[k][0])
            if base_mol is None:
                return {"error": f"failed reattaching lead R group at {k}"}

        target_dummy = _dummy_by_mapnum(base_mol, _num(target_pos))
        if target_dummy is None:
            return {"error": "target dummy not found after rebuild"}

        analogues, seen = [], set()
        for label, frag_smi in self.COMMON_R_GROUPS:
            if frag_smi is None:
                # H: cap the dummy in place
                rw = Chem.RWMol(base_mol)
                a = rw.GetAtomWithIdx(target_dummy)
                a.SetAtomicNum(1)
                a.SetIsAromatic(False)
                a.SetFormalCharge(0)
                ...
                if frag_mol is None:
                    continue
                # give the fragment a dummy at its FIRST atom's position: attach via atom 0
                combined = Chem.CombineMols(base_mol, frag_mol)
                rw = Chem.RWMol(combined)
                n_base = base_mol.GetNumAtoms()
                b_nbrs = [x.GetIdx() for x in rw.GetAtomWithIdx(target_dummy).GetNeighbors()]
                if not b_nbrs:
                    continue
                try:
                    rw.AddBond(b_nbrs[0], n_base, Chem.BondType.SINGLE)
                    rw.RemoveAtom(target_dummy)
                    m = rw.GetMol()
                    Chem.SanitizeMol(m)
                except Exception:
                    continue
            ...
            seen.add(smi)
            analogues.append({
                "r_group": label, "fragment": frag, "smiles": smi,
                "mw": round(Descriptors.MolWt(m), 2),
                "logp": round(Crippen.MolLogP(m), 2) if Crippen else None,
                "tpsa": round(rdMD.CalcTPSA(m), 1),
                "hbd": rdMD.CalcNumHBD(m), "hba": rdMD.CalcNumHBA(m),
                "rotatable": rdMD.CalcNumRotatableBonds(m),
            })
            if len(analogues) >= int(input.get("max", 60)):
                break

        analogues.sort(key=lambda a: a["mw"])
        return {
            "lead": lead, "scaffold": scaffold,
            "positions": r_keys, "position_enumerated": target_pos,
            "lead_r_groups": lead_r,
            "n_analogues": len(analogues), "analogues": analogues,
            "note": "Filter by Lipinski space (MW<=500, LogP<=5) then triage to docking/ADMET.",
        }

    def _swap_dummy_with_fragment(self, Chem, core_mol, dummy_idx, frag_mol):
        """Replace a dummy atom with the fragment mol (fragment has its own dummy to join on)."""
        core_rw = Chem.RWMol(core_mol)
        frag_rw = Chem.RWMol(frag_mol)
        # find fragment dummy and its neighbors
        frag_dummy = None
        for a in frag_rw.GetAtoms():
            if a.GetAtomicNum() == 0:
                frag_dummy = a.GetIdx()
                break
        if frag_dummy is None:
            return None
        frag_nbrs = [n.GetIdx() for n in frag_rw.GetAtomWithIdx(frag_dummy).GetNeighbors()]
        if not frag_nbrs:
            return None
        frag_attach = frag_nbrs[0]
        frag_rw.RemoveAtom(frag_dummy)
        frag_clean = frag_rw.GetMol()
        try:
            Chem.SanitizeMol(frag_clean)
        except Exception:
            frag_clean = frag_rw.GetMol()

        core_neighbors = [n.GetIdx() for n in core_rw.GetAtomWithIdx(dummy_idx).GetNeighbors()]
        if not core_neighbors:
            return None
        offset = core_rw.GetNumAtoms()
        core_rw.InsertMol(frag_clean)
        core_nbr = core_neighbors[0]
        frag_atom = offset + frag_attach
        try:
            core_rw.AddBond(core_nbr, frag_atom, Chem.BondType.SINGLE)
            core_rw.RemoveAtom(dummy_idx)
            m = core_rw.GetMol()
            Chem.SanitizeMol(m)
            return m
